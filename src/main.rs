...
use crate::{config::Config, config::{ConvertType, FormatConfig}, file_structure::AudioFile, log::Logger, opt_parser::CmdlineOptions};
...
fn main() -> Result<()> {
    ...
    match run_options.action {
            ...
            .map(|file| {
                ...
                match main_config.convert {
                    ...
                    config::ConvertType::IfNotSame => {
                        convert_this_file = file.orig_path.extension() != file.new_path.extension()
                    }
        ...
        println!(
            "{} files converted and saved successfully",
        ...
        opt_parser::Action::Preview => {
            ...
            for file in files {
                println!("{} -> {}", file.orig_path.display(), file.new_path.with_extension(encoder_config.extension.clone()).display());
            ...
            println!("{} file(s) will be transferred, with a total size of {} MB", count, size);
        },
    }
